...
import { createClient, NotFoundError } from "alcedo-sdk";
...
app.get("/health", async (_req, res) => {
  try {
    ...
    res.json({ status: "healthy", service: "hello-world-node", sdk: health });
  } catch (err) {
    res.json({ status: "degraded", service: "hello-world-node", error: err.message });
  }
});
...
app.get("/api/kv/:key", async (req, res) => {
  try {
    ...
    if (err instanceof NotFoundError) {
      res.status(404).json({ key: req.params.key, value: null, error: "not_found" });
    } else {
...
app.get("/api/kv/:key/ttl", async (req, res) => {
  try {
    ...
    if (err instanceof NotFoundError) {
      res.status(404).json({ key: req.params.key, ttl: null, error: "not_found" });
    } else {
      res.status(500).json({ key: req.params.key, error: err.message });
    }
  }
...
app.post("/api/migrate", async (_req, res) => {
  try {
    ...
    res.json({ slug: PLUGIN_SLUG, previous_status: status, message: "Migrations completed" });
  } catch (err) {
    res.status(500).json({ slug: PLUGIN_SLUG, error: err.message });
  }
});
...
app.get("/pages/dist/plugin-pages.js", async (_req, res) => {
  try {
    ...
    res.status(404).json({ error: "Pages not yet compiled — run npm run build:pages first" });
  }
});
