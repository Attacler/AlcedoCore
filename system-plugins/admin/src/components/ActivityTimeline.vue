...
const DOT_COLORS: Record<string, string> = {
  item_created: 'bg-green-500 border-green-500',
  item_updated: 'bg-blue-500 border-blue-500',
  item_deleted: 'bg-red-500 border-red-500',
}

function dotClass(action: string): string {
  return DOT_COLORS[action] || 'bg-gray-400 border-gray-400'
}
...
async function fetchTimeline() {
  ...
  try {
    ...
    const response = await client.activityLogs.listCollections(params) as {
      data: TimelineEntry[]
      total: number
    }
...
<template>
  <div class="activity-timeline">
    ...
    <div v-else-if="error" class="text-center py-8">
      ...
      <Button label="Retry" severity="danger" size="small" @click="fetchTimeline" />
    </div>
    ...
    <div v-else class="space-y-0">
      <div
        ...
        :class="{ 'border-l-2 border-gray-200': idx < entries.length - 1 }"
      ...
      >
        ...
        <div class="flex items-center gap-2 mb-1">
          <Tag :value="formatActionLabel(entry.action)" :severity="getSeverity(entry.action)" class="text-xs" />
        </div>
        <p v-if="entry.description" class="text-xs text-gray-600 mb-1">{{ entry.description }}</p>
        <p class="text-xs text-gray-400">{{ formatTimestamp(entry.created_at) }}</p>
      </div>
    </div>
