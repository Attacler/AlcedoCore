...
const emit = defineEmits(['click', 'uninstall'])
...
async function toggleEnabled() {
  ...
  try {
    ...
    toast.show(`Failed to update plugin: ${e instanceof Error ? e.message : 'Unknown error'}`, 'error')
  } finally {
    loading.value = false
  }
}
  ...
  >
    ...
    <div class="flex gap-2" @click.stop>
      <Button
        ...
        :icon="plugin.status === 'enabled' ? 'pi pi-toggle-on' : 'pi pi-toggle-off'"
        text
        severity="secondary"
        rounded
        @click="toggleEnabled"
