...
const tagPopupRef = ref<HTMLDivElement | null>(null)

const showFieldPopover = ref(false)
const showTagPopup = ref(false)
const isFocused = ref(false)
const activeFieldTag = ref<HTMLElement | null>(null)
const lastEmittedValue = ref('')

const popoverStyle = ref<Record<string, string>>({})
const tagPopupStyle = ref<Record<string, string>>({})

function parseTemplate(value: string): Array<{ type: 'text' | 'field'; value: string }> {
...
function createFieldChip(name: string): HTMLElement {
  ...
  chip.className = 'inline-flex items-center gap-1 bg-blue-100 text-blue-700 rounded px-1.5 py-0.5 text-xs font-medium leading-none'
...
function renderTemplate(value: string) {
  ...
  editorRef.value.innerHTML = segments.map(seg => {
    if (seg.type === 'field') {
...
function insertField(name: string) {
  ...
  if (startContainer.nodeType === Node.TEXT_NODE) {
    ...
    if (beforeCursor.endsWith('{{')) {
      startContainer.textContent = text.slice(0, offset - 2) + text.slice(offset)
      range.setStart(startContainer, offset - 2)
      range.collapse(true)
    }
  }
...
function onDocumentMousedown(event: MouseEvent) {
  ...
  if (showFieldPopover.value && popoverRef.value) {
    if (!popoverRef.value.contains(target) && !editorRef.value?.contains(target)) {
      showFieldPopover.value = false
    }
  }
  if (showTagPopup.value && tagPopupRef.value) {
    if (!tagPopupRef.value.contains(target) && !(target.closest('[data-field]'))) {
      showTagPopup.value = false
      activeFieldTag.value = null
    }
  }
    ...
    >
      <template v-if="fields.length > 0">
        ...
        >
          ...
          <span class="text-xs text-gray-400 ml-auto">{{ field.type }}</span>
        </div>
      </template>
      <div v-else class="px-3 py-2 text-sm text-gray-400">No fields available</div>
    </div>
    <div
      v-if="showTagPopup"
      ref="tagPopupRef"
