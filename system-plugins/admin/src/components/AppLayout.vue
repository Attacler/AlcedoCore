...
const activeSection = ref<'content' | 'settings'>('content')
const expandedSubmenus = ref(new Set<string>())
const logoError = ref(false)

/** Sections filtered by the active section toggle (Browse vs Settings) */
...
const settingsSections = computed<MenuSection[]>(() => {
  ...
  sections.push({
    ...
    visible: true,
    items: [
      { id: 'dashboard', label: 'Dashboard', icon: 'home', route: '/dashboard', visible: true },
      { id: 'plugins-overview', label: 'Plugins', icon: 'extension', route: '/plugins', visible: true },
      { id: 'media', label: 'Media', icon: 'folder', route: '/files', visible: true },
      { id: 'registries', label: 'Registries', icon: 'cloud', route: '/registries', visible: true },
      { id: 'policies', label: 'Policies', icon: 'policy', route: '/policies', visible: true },
      { id: 'users', label: 'Users', icon: 'group', route: '/users', visible: true },
      { id: 'roles', label: 'Roles', icon: 'security', route: '/roles', visible: true },
      { id: 'settings-link', label: 'Settings', icon: 'settings', route: '/settings', visible: true },
    ],
  })

  const collectionItems: MenuItem[] = [
    { id: 'collections-overview', label: 'Overview', icon: 'list', route: '/collections', visible: true },
  ]
...
const branding = computed(() => ({
  siteName: settingsStore.getSettingValue('site_name') || 'Admin Dashboard',
...
watch(() => branding.value.faviconUrl, (url) => {
...
watch(() => branding.value.logoUrl, () => {
  logoError.value = false
})

/**
 * Filter items to only visible ones (MENU-05: visibility toggle preserves hidden items).
 */
 ...
 */
function resolveItemRoute(item: { route?: string; url?: string; external?: boolean }) {
...
onMounted(() => {
  ...
  // Load user-created menu sections from settings API
  menuStore.loadMyMenus()

  // Load branding settings for title/logo/favicon
  settingsStore.fetchSettings()

  // Fetch collections and plugin pages for dynamic content items
  Promise.all([
    collectionsStore.fetchCollections(),
    pluginsStore.fetchPlugins(),
  ]).then(async () => {
    const collections = collectionsStore.collections
    const dynamicItems: Array<{ id: string; label: string; icon: string; route?: string; visible: boolean }> = []

    // Collections → dynamic nav items
    ...
    // Plugin pages → dynamic nav items
    const enabledPlugins = pluginsStore.plugins.filter(p => p.status === 'enabled')
    for (const plugin of enabledPlugins) {
      try {
        const pluginPages = await pluginsStore.fetchPluginPages(plugin.name)
        for (const p of pluginPages.filter((p: any) => p.sidebar)) {
          dynamicItems.push({
            id: `plugin-page-${plugin.name}-${p.path}`,
            label: p.label,
        ...
        console.warn(`Failed to fetch pages for plugin ${plugin.name}:`, e)
      }
    }

    // Pass dynamic items to menuStore for merged menu computation
    menuStore.setDynamicItems(dynamicItems)
  })

  // Discover plugin views for the collection items overview
...
<template>
  <div class="flex flex-col min-h-screen bg-gray-100">
    ...
    >
      ...
      <div class="h-16 flex items-center justify-between px-3 border-b border-slate-700">
        <div class="flex items-center gap-2 min-w-0">
          <img
            v-if="branding.logoUrl && !logoError"
            :src="branding.logoUrl"
            alt="Logo"
            class="h-8 w-auto max-w-[160px] object-contain"
            @error="logoError = true"
          />
          <span v-else class="material-symbols-outlined text-2xl">dashboard</span>
          <span v-if="!collapsed" class="text-sm font-semibold truncate">{{ branding.siteName }}</span>
        </div>
        <Button
          v-if="!isMobile"
      ...
      <template v-if="activeSection === 'content' && menuStore.menus.length > 1">
        <div class="px-2 py-2 border-b border-slate-700/50 menu-switcher-area">
          <div class="relative">
            <button
            ...
            >
              <span class="material-symbols-outlined text-lg">{{ menuStore.activeMenu?.icon || 'menu' }}</span>
              <span v-if="!collapsed" class="flex-1 text-left truncate">{{ menuStore.activeMenu?.name || 'No Menu' }}</span>
              <span v-if="!collapsed" class="material-symbols-outlined text-sm ml-auto">arrow_drop_down</span>
            </button>
            <!-- Dropdown -->
            <div
            ...
            >
              <div
                ...
                :class="{ 'bg-slate-700/70': m.id === menuStore.activeMenuId }"
              >
                <span class="material-symbols-outlined text-lg">{{ m.icon }}</span>
                <span>{{ m.name }}</span>
                <span v-if="m.id === menuStore.activeMenuId" class="ml-auto text-xs text-blue-400">Active</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <nav class="flex flex-col gap-1 p-2 flex-1 overflow-y-auto">
        ...
        <div v-for="section in filteredSections" :key="section.id" class="mb-2">
          <!-- Section Header -->
          <div
            v-if="!collapsed"
          ...
          <!-- Section Items -->
          <template v-for="item in visibleSectionItems(section)" :key="item.id">
            <!-- If item has children, render as collapsible group -->
            <template v-if="item.children && item.children.length > 0">
              <div
                class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-700 transition-colors cursor-pointer border-l-2 border-transparent"
                :class="{ 'flex-col': !isMobile && !collapsed, 'text-sm': isMobile }"
                @click="toggleSubmenu(item.id)"
              >
                <span class="material-symbols-outlined text-xl">{{ item.icon }}</span>
                <span v-if="!collapsed" class="text-xs">{{ item.label }}</span>
                <span v-if="!collapsed" class="material-symbols-outlined text-sm ml-auto transition-transform" :class="{ 'rotate-90': expandedSubmenus.has(item.id) }">
                  chevron_right
                </span>
              </div>
              <!-- Children (submenu) -->
              <div v-if="expandedSubmenus.has(item.id)" class="ml-4 border-l border-slate-600 pl-2">
                <router-link
                ...
                >
                  ...
                  <span v-if="!collapsed" class="text-xs">{{ child.label }}</span>
                  <span v-if="child.external" class="material-symbols-outlined text-xs text-slate-400 ml-1">open_in_new</span>
                </router-link>
              </div>
            </template>

            <!-- Regular item (no children) — render as router-link or external link -->
            <router-link
              v-else
              :key="item.id"
            ...
            >
              <span class="material-symbols-outlined text-xl">{{ item.icon }}</span>
              <span v-if="!collapsed" class="text-xs">{{ item.label }}</span>
              <span v-if="item.external" class="material-symbols-outlined text-xs text-slate-400 ml-1">open_in_new</span>
            </router-link>
          </template>

          <!-- Empty section placeholder -->
          <div v-if="section.visible && visibleSectionItems(section).length === 0 && !collapsed" class="px-3 py-2 text-xs text-slate-500 italic">
            No items
          </div>
        </div>
      </nav>
      ...
      <!-- User Info & Logout -->
      <div v-if="authStore.user" class="border-t border-slate-700 px-3 py-2">
      ...
      <div class="border-t border-slate-700 p-2">
        <div :class="collapsed ? 'flex flex-col gap-1' : 'flex gap-1'">
          <button
            ...
            :class="activeSection === 'content' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-700 hover:text-white'"
          >
            <span class="material-symbols-outlined text-lg">browse</span>
            <span v-if="!collapsed">Browse</span>
          </button>
          <button
            v-if="authStore.scopes.some(s => s === 'users.all' || s === 'settings.all' || s === 'settings.read.all' || s === 'settings.write.all')"
            @click="switchSection('settings')"
            class="flex-1 flex items-center justify-center gap-1 px-2 py-2 rounded-lg text-xs transition-colors cursor-pointer"
            :class="activeSection === 'settings' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-700 hover:text-white'"
          >
    ...
    <header class="lg:hidden sticky top-0 z-30 bg-white shadow-sm">
      <div class="h-14 flex items-center px-4 gap-3">
        <button @click="toggleSidebar" class="p-2 -ml-2 rounded-lg hover:bg-gray-100 cursor-pointer">
          <span class="material-symbols-outlined text-xl">menu</span>
        </button>
        <h1 class="text-lg font-semibold text-gray-800">{{ branding.siteName }}</h1>
      </div>
    ...
    <!-- Main Content Area -->
    <div class="flex-1 flex flex-col min-h-dvh transition-all duration-300" :class="{ 'lg:ml-20': !isMobile && collapsed, 'lg:ml-64': !isMobile && !collapsed }">
      <!-- Scrollable Content -->
