...
import { usePluginsStore, type InstanceDetail, type ContainerStatsSnapshot } from '@/stores/plugins'
...
const statsHistory = ref<ContainerStatsSnapshot[]>([])
...
async function loadData() {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : 'Failed to load instance details'
  } finally {
    loading.value = false
  }
}
...
async function restartInstance() {
  ...
  try {
    await store.restartPlugin(props.pluginName, detail.value?.container_id || undefined)
    await loadData()
  } catch (e) {
...
async function fetchStats() {
  if (!props.taskId || !props.pluginName) return
  try {
    const stats = await store.fetchInstanceStats(props.pluginName, props.taskId)
...
const memChartOptions = {
  ...
  scales: {
    ...
    y: {
      beginAtZero: true,
      ticks: { font: { size: 10 }, callback: (v: any) => `${formatFileSize(v)}` },
    },
  },
}
  ...
  >
    ...
    <template v-else-if="error">
      <div class="text-center text-red-500 py-8">
        ...
        <Button label="Retry" severity="warn" size="small" @click="loadData" />
      </div>
    </template>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-gray-50 rounded-lg p-4 space-y-2">
          <div class="flex items-center justify-between">
            ...
            <span
              ...
              :class="{
                ...
                'bg-gray-100 text-gray-700': !detail.status || detail.status === 'shutdown'
              }"
            >{{ detail.status }}</span>
          </div>
          ...
          <div class="flex justify-between text-sm">
            ...
            <span class="capitalize">{{ detail.desired_state }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">Container ID</span>
            <span class="font-mono text-xs truncate max-w-[200px]">{{ detail.container_id || '—' }}</span>
          </div>
          <div class="pt-3">
            <Button
        ...
        <!-- Container Details -->
        <div v-if="detail.container" class="bg-gray-50 rounded-lg p-4 space-y-2">
          <h4 class="text-sm font-semibold text-gray-700 mb-2">Container</h4>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">Name</span>
            <span class="font-mono text-xs truncate max-w-[220px]">{{ detail.container.name }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">State</span>
            <span :class="{ 'text-green-600': detail.container.state === 'running', 'text-red-600': detail.container.state !== 'running' }">
              {{ detail.container.state }}
            </span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">Image</span>
            <span class="font-mono text-xs truncate max-w-[220px]">{{ detail.container.image }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">Created</span>
            <span class="text-xs">{{ formatDate(detail.container.created) }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">Network</span>
            <span class="text-xs">{{ detail.container.network_mode || 'default' }}</span>
          </div>
        </div>

        <!-- CPU Usage Chart -->
        <div class="bg-gray-50 rounded-lg p-4">
          <h4 class="text-sm font-semibold text-gray-700 mb-3">CPU Usage (%)</h4>
          <div v-if="cpuChartData" class="relative" style="height: 300px;">
            <Chart type="line" :data="cpuChartData" :options="chartOptions" />
          </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4">
          ...
          <div v-if="memChartData" class="relative" style="height: 300px;">
            <Chart type="line" :data="memChartData" :options="memChartOptions" />
          </div>
          <div v-else class="flex items-center justify-center h-32 text-gray-400 text-sm">
            Collecting data...
          </div>
