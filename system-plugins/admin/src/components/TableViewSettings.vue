...
<template>
  <div class="space-y-4 p-4">
    <h3 class="text-lg font-semibold text-gray-800 text-sm">Table Columns</h3>
    <p class="text-xs text-gray-400">Select which fields to show in the table view.</p>
    <div class="space-y-1 max-h-64 overflow-y-auto">
      <label v-for="f in visibleFields" :key="f.name"
        ...
        <Checkbox :binary="true" :model-value="isSelected(f.name)" @change="toggle(f.name)" />
    ...
    <p class="text-xs text-gray-400 pt-2 border-t border-gray-100">
      {{ selectedCount }} of {{ visibleFields.length }} fields selected
    </p>
  </div>
</template>