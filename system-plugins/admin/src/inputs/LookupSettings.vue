...
const relatedCollectionFields = ref<{ name: string; display_name?: string; type: FieldType }[]>([])

const availableCollections = computed(() =>
  allCollections.value.filter(c => c.name !== props.collectionName)
)
...
const inlineParentFieldOptions = computed(() =>
  relatedCollectionFields.value.filter(f => f.type !== 'relationship')
)

watch(() => props.field?.related_collection, async (rc) => {
  if (!rc) {
    relatedCollectionFields.value = []
    return
  }
...
function toggleInlineParentField(fieldName: string) {
  ...
  if (current.includes(fieldName)) {
    props.field.inline_parent_fields = current.filter((f: string) => f !== fieldName)
  } else {
...
<template>
  <div v-if="field" class="space-y-3 pt-4 border-t border-gray-200">
    <h4 class="text-sm font-semibold text-gray-700">Relationship Settings</h4>
    <div>
      <label class="block text-xs font-medium text-gray-600 mb-1">Related Collection</label>
      <Select
    ...
    <div v-if="field.related_collection">
      <label class="block text-xs font-medium text-gray-600 mb-1">
        {{ field.relationship_type === 'one_to_many' ? 'Display Field on Child' : 'Display Field' }}
      </label>
      <Select
        :modelValue="field.display_field"
        @update:modelValue="field.display_field = $event || null"
    ...
    <div v-if="field.related_collection" class="pt-3 border-t border-gray-100">
      <label class="block text-xs font-medium text-gray-600 mb-1">Inline Parent Fields</label>
      <div class="space-y-1 max-h-32 overflow-y-auto">
        <label v-for="f in inlineParentFieldOptions" :key="f.name" class="flex items-center gap-2 cursor-pointer">
          <Checkbox
            :binary="true"
            :modelValue="isInlineParentFieldSelected(f.name)"
            @update:modelValue="toggleInlineParentField(f.name)"
          />
          <span class="text-sm text-gray-700"><FieldNameLabel :field="f" /></span>
        </label>
      </div>
      <p class="text-xs text-gray-400 mt-1">Show parent fields inline on child record detail</p>
    </div>
  </div>
</template>
