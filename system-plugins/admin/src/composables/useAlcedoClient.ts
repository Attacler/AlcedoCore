...
export function useAlcedoClient(devServerUrl?: string) {
  const client = createClient(typeof window !== 'undefined' ? window.location.origin : '')
...
export { AlcedoError, NotFoundError, ValidationError, AuthenticationError, ServerError, ConnectionError }
