...
const registryId = computed(() =>
    ...
    isEdit = computed(
        () => registryId.value !== null && !isNaN(registryId.value),
    );

const loading = ref(false),
    saving = ref(false),
    showDeleteConfirm = ref(false),
    reachableStatus = ref<"unknown" | "checking" | "reachable" | "unreachable">(
        "unknown",
    );
...
async function checkReachability(showToast: boolean) {
...
async function saveRegistry() {
    ...
    if (registryId.value) {
        if (reachableStatus.value === "unknown") {
            await checkReachability(false);
            if ((reachableStatus.value as string) !== "reachable") {
                toast.show(
                    "Please wait for registry validation to complete",
                    "warning",
                );
                return;
            }
...
<template>
    <div class="p-6 max-w-2xl">
        <router-link
            ...
            >← Back to Registries</router-link
        >
            ...
            <div class="p-6 border-b border-gray-200 space-y-4">
                ...
                <div>
                    ...
                    <InputText
                        ...
                        fluid
                        :invalid="!!errors.url"
                        @blur="checkReachability(true)"
                    />
                    <div class="flex items-center gap-2 mt-1">
                        <p v-if="errors.url" class="text-xs text-red-500">
                            {{ errors.url }}
                        </p>
                        <span
                            v-if="reachableStatus === 'checking'"
                            class="text-xs text-gray-500"
                            >Checking reachability...</span
                        >
                        <span
                            v-else-if="reachableStatus === 'reachable'"
                        ...
                        <span
                            v-else-if="reachableStatus === 'unreachable'"
                ...
                <div>
                    ...
                    <InputText
                        ...
                        type="url"
                        placeholder="e.g. localhost:5000 or k8s-side:5001"
                        class="w-full"
                        fluid
                    />
                    <p class="text-xs text-gray-500 mt-1">
                        If set, container image pulls use this hostname instead
                        of the URL above. Use when the registry API is reachable
                        at a different hostname than the container runtime.
                    </p>
                </div>
