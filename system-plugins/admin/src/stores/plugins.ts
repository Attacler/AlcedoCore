...
import type { Plugin, PluginSchemaResponse, MigrationStatus as SdkMigrationStatus, PluginPage as SdkPluginPage } from 'alcedo-sdk'
import { useAlcedoClient } from '../composables/useAlcedoClient'
import { withAsyncHandlingVoid } from '../utils/asyncUtils'

// Re-export types for external use
export type { Plugin, PluginSchemaResponse, MigrationStatus } from 'alcedo-sdk'
...
function mapBackendPlugin(backendPlugin: { slug?: string; name?: string; version: string; enabled?: boolean; status?: string; type?: string; plugin_type?: string; description?: string; created_at?: string; updated_at?: string; endpoints?: Record<string, EndpointInfo>; documentation?: string[] }): PluginStore {
  return {
    name: backendPlugin.slug || backendPlugin.name || '',
    version: backendPlugin.version,
    status: backendPlugin.status === 'running' ? 'enabled' : (backendPlugin.status === 'stopped' ? 'disabled' : (backendPlugin.enabled ? 'enabled' : 'disabled')),
    plugin_type: (backendPlugin.plugin_type === 'static' || backendPlugin.type === 'static') ? 'system' : 'user',
  ...
  return {
    tables: backendSchema.tables.map(t => ({
      ...
      primaryKey: t.primary_key
        ? { constraintName: t.primary_key.constraint_name, columns: t.primary_key.columns }
        : undefined,
...
function mapMigration(backendMigration: BackendMigrationStatus): SdkMigrationStatus {
  return {
    ...
    appliedAt: backendMigration.status === 'applied' ? backendMigration.applied_at : null,
...
export const usePluginsStore = defineStore('plugins', () => {
  ...
  async function fetchPlugins() {
    await withAsyncHandlingVoid(loading, error, async () => {
      ...
      const pluginList = (response as any)?.data?.plugins || (response as any)?.plugins || []
  ...
  async function addFromRegistry(slug: string, image: string): Promise<PluginStore> {
    const data = await client.plugins.createFromRegistry({ slug, image, status: 'disabled' }) as any
    await fetchPlugins()
    return data.data
  }
  ...
  async function fetchPluginMigrations(name: string): Promise<{ migrations: SdkMigrationStatus[] }> {
    ...
    const data = (response as { migrations?: BackendMigrationStatus[] }).migrations || []
    return { migrations: data.map(mapMigration) }
  }

  async function rollbackMigration(name: string, version: string): Promise<any> {
    return await (client.migrations as any).rollback(name, version)
  }

  async function fetchPluginAssets(name: string): Promise<{ css: string; js: string }> {
  ...
  async function fetchPluginLogs(
    ...
    const response = await client.plugins.requestLogs(name, params) as any
  ...
  async function fetchPluginLogDetail(name: string, requestId: string): Promise<LogDetailResponse> {
    const response = await client.plugins.requestLogDetail(name, requestId) as any
  ...
  async function fetchPluginVersions(name: string): Promise<ListVersionsResponse> {
    const response = await client.plugins.versions(name) as any
    return response.data || { versions: [] }
  }

  // VERSIONS-02/API: deploy a specific version by tag
  async function deployPluginVersion(name: string, tag: string): Promise<{ data?: { status: string } }> {
  ...
  async function fetchInstanceDetail(slug: string, taskId: string): Promise<InstanceDetail> {
  ...
  async function fetchInstanceStats(slug: string, taskId: string): Promise<ContainerStatsSnapshot> {
    const json = await client.plugins.instanceStats(slug, taskId) as any
    return json.data
  }

  // INSTANCES-04/API: get logs for a specific instance
  async function fetchInstanceLogs(slug: string, taskId: string): Promise<string[]> {
  ...
  async function scalePlugin(slug: string, replicas: number, resourceLimits?: { cpu_limit: number; memory_limit: number }): Promise<void> {
    await client.plugins.scale(slug, { replicas, resource_limits: resourceLimits })
  }

  async function restartPlugin(name: string, containerId?: string): Promise<any> {
  ...
  async function updatePluginScopes(slug: string, scopes: string[]): Promise<void> {
  ...
  return {
    ...
    fetchPlugins, enablePlugin, disablePlugin, uninstallPlugin, deletePlugin, installPlugin, createPlugin, addFromRegistry,
    fetchPluginDetail, fetchPluginSchema, fetchPluginMigrations,     fetchPluginAssets, fetchPluginPages, getCachedPluginPages, pluginPagesMap, savePluginSettings, fetchPluginSettings,
    fetchPluginDocs, fetchDocContent,
    fetchPluginLogs, fetchPluginLogDetail, fetchPluginDockerInfo,
    fetchPluginVersions, deployPluginVersion,
    rollbackMigration,
    fetchPluginInstances, fetchInstanceDetail, fetchInstanceStats, fetchInstanceLogs, scalePlugin,
    fetchPluginScopes, updatePluginScopes,
    developerApiKeys, restartPlugin,
  }
})