...
async function saveCollectionDetails() {
    ...
    try {
        ...
        await store.updateCollection(collectionMeta.value.name, {
            fields: payload,
            display_name: collectionDisplayName.value || null,
        });
        collectionMeta.value.display_name =
...
const availableDisplayTypes = computed(() => {
    ...
    types.push(
        ...DISPLAY_TYPE_REGISTRY.filter(
            (e) => e.dbType === effectiveDbType && e.type !== pkType,
...
function onDragOverEmpty() {
    isDragging.value = true;
}
function onDragLeaveEmpty() {
    isDragging.value = false;
}
...
function commitDraft(key: string) {
    ...
    if (!f.name || !/^[a-z][a-z0-9_]*$/.test(f.name)) {
        toast.show(
            "Field name is required — use only lowercase letters, numbers, and underscores",
            "error",
        );
        return;
    }
...
async function saveLayout() {
    ...
    try {
        ...
        await loadLayouts();
        activeLayoutId.value =
            collLayouts.value[collLayouts.value.length - 1]?.id || null;
...
function toggleFieldInSection(fieldKey: string) {
    ...
    if (current.includes(field.name)) {
        sectionFormData.value.display_fields = current.filter(
            (f: string) => f !== field.name,
        );
    } else {
...
<template>
    <div class="p-6">
            ...
            <div class="flex justify-between items-start mb-6">
                <div>
                    ...
                    <h1 class="text-2xl font-bold text-gray-900">
                        <CollectionNameLabel
                            :collection="collectionMeta as any"
                        />
                    </h1>
                </div>
                <div class="flex gap-2">
                    <Button
                        v-if="!collectionMeta?.is_system"
                        label="Browse Data"
                        severity="secondary"
                        outlined
                        as="router-link"
            ...
            <div class="flex gap-6">
                ...
                <div class="w-64 flex-shrink-0">
                    ...
                    >
                        ...
                        <div class="p-3 space-y-3">
                            ...
                            >
                                <div
                                    class="text-xs font-semibold text-gray-500 px-1 mb-1 uppercase tracking-wider"
                                >
                                    {{ group.label }}
                                </div>
                                <div class="grid grid-cols-2 gap-1">
                                    ...
                                    >
                                        <span
                                            class="w-4 h-4 flex items-center justify-center text-xs text-gray-400 shrink-0"
                                            v-html="item.icon"
                                        ></span>
                                        <span class="truncate">{{
                                            item.label
                        ...
                        <div
                            class="border-t border-gray-200 px-4 py-2 text-xs text-gray-400"
                        >
                            {{ namedFieldCount }} field{{
                                namedFieldCount !== 1 ? "s" : ""
                            }}
                            defined
                        </div>
                ...
                <div class="flex-1">
                    <div
                        ...
                        class="flex items-center justify-center h-64 bg-white rounded-lg border-2 border-dashed border-gray-300"
                    ...
                    >
                        ...
                        <div
                            v-if="unnamedFields.length > 0"
                            class="border-b border-yellow-200 bg-yellow-50/50"
                        >
                            <div
                                class="px-4 py-2 text-xs font-medium text-yellow-700 border-b border-yellow-200 flex items-center gap-2"
                            >
                                <span>&#9888;</span>
                                <span
                                    >{{ unnamedFields.length }} unnamed field{{
                                        unnamedFields.length !== 1 ? "s" : ""
                                    }}</span
                                >
                            </div>
                            <div class="grid grid-cols-2 gap-4 p-4">
                                <div
                                    v-for="field in unnamedFields"
                                    :key="field._key"
                                ...
                                >
                                    ...
                                    <InputText
                                        ...
                                        :value="field.name"
                                        @input="
                                            onDraftNameInput(field._key, $event)
                                        "
                                        @keydown.enter="commitDraft(field._key)"
                                        maxlength="59"
                        ...
                        >
                            ...
                            <div class="flex gap-1">
                                ...
                                <Button
                                    v-if="
                                        collLayouts.length > 1 && activeLayoutId
                                    "
                                    icon="pi pi-trash"
                                    severity="danger"
                                    size="small"
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <template
                                        v-if="
                                            getSectionFields(section).length ===
                                            0
                                        "
                                    >
                                        <div
                                            ...
                                            "
                                            @dragover.prevent="
                                                onDragOverEmptySection(section)
                                            "
                                            @dragleave="
                                                onDragLeaveEmptySection(section)
                                            "
                                            @drop="
                                                onDropInSection($event, section)
                                            "
                                        >
                                            <template
                                                v-if="dragType || dragFieldKey"
                                    ...
                                    <template v-else>
                                                    ...
                                                    <div
                                                        ...
                                                        :class="
                                                            ...
                                                            field._key
                                                                ...
                                                                : isDragging
                                                                  ? 'border-2 border-dashed border-transparent hover:border-blue-300 hover:bg-blue-50 cursor-pointer'
                                                                  : ''
                                                        "
                                                        @dragover.prevent="
                                                            onDragOverField(
                                        ...
                                        >
                                            <template
                                                v-for="field in flatSectionFields(
                                                    section,
                                                )"
                                                :key="field._key"
                                            >
                        ...
                        <div v-if="sectionFormData" class="space-y-4">
                            ...
                            >
                                <div>
                                    ...
                                    >
                                    <div
                                        class="border border-gray-200 rounded-md p-3 space-y-1 max-h-48 overflow-y-auto"
                                    >
                                        <label
                                            v-for="field in availableFieldsForSection"
                                            :key="field._key"
                                        ...
                                        >
                                            <Checkbox
                                                :binary="true"
                                                :model-value="
                                                    isFieldInSection(field._key)
                                                "
                                                @change="
                                                    toggleFieldInSection(
                                                        field._key,
                                                    )
                                                "
                                            />
                                        ...
                                        <div
                                            v-if="
                                                availableFieldsForSection.length ===
                                                0
                                            "
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">
                                        Check fields to include in this section
                                    </p>
                                </div>
                                <!-- Columns layout -->
                                <div class="border-t pt-3 mt-3">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            "
                                            size="small"
                                            @click="sectionColumns = 2"
                                            class="flex-1"
                                        />
                                    </div>
                                    <div
                                        v-if="sectionColumns === 2"
                                        class="mt-2"
                                    >
                                        <p class="text-xs text-gray-400">
                                            Drag fields between columns to
                                            assign them. Each field keeps its
                                            column assignment.
                                        </p>
                                    </div>
                                </div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <p class="text-xs text-gray-400 mb-2">
                                        Only show items matching these
                                        conditions.
                                    </p>
                                    <FilterBuilder
                                ...
                                >
                                    ...
                                    >
                                    <p class="text-xs text-gray-400 mb-2">
                                        Only show this section when conditions
                                        are met.
                                    </p>
                                    <div
                                    ...
                                    >
                                        <div>
                                            ...
                                            <FilterBuilder
                                                v-model="
                                                    sectionFormData.visibility_parent
                                                "
                                                :fields="namedFields"
                                                :collection-name="
                                                    collectionName
                                        ...
                                        <div class="border-t pt-3">
                                            ...
                                            <FilterBuilder
                                                v-model="
                                                    sectionFormData.visibility_child
                                                "
                                                :fields="childCollectionFields"
                                                :collection-name="
                                                    getChildCollectionName(
                ...
                <div v-if="editingField" class="space-y-4">
                        ...
                        <div class="flex gap-4">
                            ...
                            >
                                <Checkbox
                                    :binary="true"
                                    :model-value="
                                        (editingField as any).full_width
                                    "
                                    @update:model-value="
                                        (val: any) => {
                                            (editingField as any).full_width =
                    ...
                    >
                        ...
                        <InputText
                            ...
                            fluid
                            :placeholder="
                                defaultValuePlaceholder(editingField.type)
                            "
                        />
                    </div>
                    <div>
                        ...
                        >
                        <Select
                            :value="editingField.display_type || 'default'"
                    ...
                    >
                        ...
                        <div>
                            ...
                            <InputNumber
                                v-model="
                                    (editingField as any).options.max_file_size
                                "
                                :min="0"
                                :step="1048576"
                                class="w-full"
                        ...
                        <div>
                            ...
                            />
                            <p class="text-xs text-gray-400 mt-1">
                                Leave empty to allow all types. Use glob
                                patterns like image/*
                            </p>
                        </div>
                    </div>
        ...
        <Dialog
            ...
            @update:visible="(v) => (showLayoutDialog = v)"
            header="New Layout"
            :modal="true"
            :style="{ width: '400px' }"
            :draggable="false"
        ...
        <!-- Layout Roles Dialog -->
        <Dialog
            :visible="showLayoutRolesDialog"
            @update:visible="(v) => (showLayoutRolesDialog = v)"
        ...
        >
            <div class="space-y-3">
                ...
                <div
                    class="flex justify-end gap-2 pt-2 border-t border-gray-200"
                >
                    <Button
                        label="Cancel"
                        severity="secondary"
