...
const categoryDef = computed(() => CATEGORIES.find(c => c.id === props.category))
...
async function onFileSelected(key: string, event: Event) {
  ...
  try {
    ...
    toast.show(`Upload failed: ${e instanceof Error ? e.message : 'Unknown error'}`, 'error')
  } finally {
...
<template>
  ...
  <div v-else class="space-y-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <div class="flex items-center gap-2">
            <span v-if="categoryDef" class="material-symbols-outlined text-gray-500">{{ categoryDef.icon }}</span>
            <h1 class="text-2xl font-semibold text-gray-900">{{ categoryDef?.label || 'Settings' }}</h1>
          </div>
          <p v-if="categoryDef" class="text-sm text-gray-500 ml-8">{{ categoryDef.description }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="settingsKeys.length === 0 && category !== 'developer'" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
      <span class="material-symbols-outlined text-4xl text-gray-300 mb-3">{{ categoryDef?.icon || 'settings' }}</span>
    ...
    <!-- Developer: localStorage toggle -->
    <div v-else-if="category === 'developer'" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        >
          ...
          <div class="flex items-center gap-3 flex-shrink-0 max-w-[320px] w-full sm:max-w-[280px]">
            <div class="flex-1 min-w-0">
              <Select v-if="SETTING_META[key]?.type === 'select'" :modelValue="store.getLocalValue(key)" @update:modelValue="store.updateLocalValue(key, $event)" :options="store.dynamicOptions[key] || SETTING_META[key]?.options" option-label="label" option-value="value" class="w-full" :invalid="!!store.validateValue(key)" />
              ...
              <div v-else-if="SETTING_META[key]?.type === 'file-image'" class="flex items-center gap-3">
                <img v-if="store.getLocalValue(key)" :src="store.getLocalValue(key)" class="w-10 h-10 object-contain rounded border border-gray-200" />
                <span v-else class="w-10 h-10 flex items-center justify-center rounded border border-gray-200 bg-gray-50 text-gray-400 text-xs">No image</span>
                <Button icon="pi pi-upload" severity="secondary" outlined size="small" @click="openFilePicker(key)" :loading="uploadingKey === key" />
                <Button v-if="store.getLocalValue(key)" icon="pi pi-trash" severity="danger" text size="small" @click="store.updateLocalValue(key, '')" />
              </div>
