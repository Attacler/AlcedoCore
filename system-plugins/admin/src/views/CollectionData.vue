...
import { ref, computed, markRaw, defineAsyncComponent, onMounted, onUnmounted, watch, type Component } from 'vue'
...
async function fetchCreatePolicy() {
  ...
  try {
    const policy = await client.collections.getCreatePolicy(collectionName.value) as any
...
const activeFilterCount = computed(() => countFilterConditions(filterCondition.value))
...
function countFilterConditions(cond: FilterCondition | null): number {
  ...
  if ('conditions' in cond) return cond.conditions.reduce((sum, c) => sum + countFilterConditions(c), 0)
  return 0
}
...
const displayFieldNames = computed(() => {
  ...
  if (view?.config?.view_specific?.displayFields && Array.isArray(view.config.view_specific.displayFields) && view.config.view_specific.displayFields.length > 0)
    return view.config.view_specific.displayFields as string[]
  return undefined
})

const currentViewSettings = computed(() => {
  if (Object.keys(localViewSettings.value).length > 0) return localViewSettings.value
  const view = savedViewsStore.activeView
...
const currentViewConfig = computed(() => ({
  ...
  sort: sortField.value ? { field: sortField.value, order: sortOrder.value } : undefined,
  filters: { ...legacyFilters.value },
  filterCondition: filterCondition.value ? { ...filterCondition.value } : undefined,
...
const viewModeOptions = computed(() => {
  ...
  const extViews = extensionRegistry.allViewTypes.filter(v =>
    !['table', 'cards', 'kanban'].includes(v.type)
  )
  if (extViews.length > 0) {
    options.push({ value: '---separator2---', label: '──────────', disabled: true })
...
const activeViewMode = computed(() => {
  ...
  if (savedViewsStore.activeView?.config.render_mode) return savedViewsStore.activeView.config.render_mode
...
const currentViewModeLabel = computed(() => {
  const opt = viewModeOptions.value.find(o => o.value === activeViewMode.value)
...
function onViewModeSelect(value: string) {
  ...
  if (savedViewsStore.activeView?.id) {
    savedViewsStore.updateView(collectionName.value, savedViewsStore.activeView.id, {
      config: {
...
const systemFieldKeys = computed(() => {
  ...
  return SYSTEM_FIELD_NAMES.filter(k => k in items.value[0] && !fieldNames.has(k))
})
...
async function fetchItems() {
  ...
  try {
    ...
    const res = await client.items.list(collectionName.value, paramsObj) as any
...
async function fetchRelatedFieldOptions() {
  const relFields = fields.value.filter(f => f.type === 'relationship' && f.related_collection)
  ...
  for (const rf of relFields) {
    ...
    try {
      ...
      console.warn(`[CollectionData] Could not fetch related collection: ${relName}`)
    }
  }
...
function applyViewConfig(view: { config: { sort?: { field: string; order: string }; filters?: Record<string, string>; filterCondition?: FilterCondition | null } }) {
  overrideRenderMode.value = null
...
watch(() => settingsStore.settings['default_page_size'], (newVal) => {
...
watch(() => savedViewsStore.activeViewId, (newId) => {
...
watch(() => localViewSettings.value.sortField, (newVal) => {
  if (newVal) {
...
watch(() => localViewSettings.value.sortOrder, (newVal) => {
...
<template>
  <div class="p-6">
    ...
    <div class="mb-6">
      ...
      <div v-if="metaLoading" class="text-gray-500 text-lg">Loading collection...</div>
      <div v-else class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900"><CollectionNameLabel :collection="collection!" /></h1>
        <Button
          v-if="canCreate"
          label="Add new item"
          icon="pi pi-plus"
    ...
    <div v-if="collection?.is_system && !metaLoading" class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg text-center">
      <p class="text-blue-700">
        Data for this collection is managed through its dedicated interface.
      </p>
    </div>

    <!-- Toolbar: grouped icon-only controls -->
    <div v-if="!metaLoading && !collection?.is_system" class="mb-4 bg-white rounded-lg shadow-sm">
      <div class="flex flex-wrap sm:flex-nowrap items-center gap-1 sm:gap-3 p-2 sm:p-3">
        <!-- Group 1: View management -->
        <div class="flex flex-wrap items-center gap-1 sm:gap-2 w-full sm:flex-1 min-w-0">
          <ViewSelector
          ...
          <div class="relative">
            ...
            >{{ currentViewModeLabel }} <i class="pi pi-chevron-down text-[10px]"></i></button>
            <div
            ...
            >
              <template v-for="opt in viewModeOptions" :key="opt.value">
                ...
                <button
                  ...
                  :class="opt.value === activeViewMode ? 'text-blue-600 font-medium bg-blue-50' : 'text-gray-700'"
        ...
        <div class="flex items-center gap-1">
          ...
          <Button
            ...
            size="small"
            :title="hasActiveFilters ? `Filters (${activeFilterCount} active)` : 'Filters'"
    ...
    >
      <div class="flex items-center justify-between mb-2 sm:mb-3">
        ...
        <div class="flex items-center gap-2">
          <span v-if="hasActiveFilters" class="text-xs text-blue-600 bg-blue-50 rounded px-2 py-1 border border-blue-200">
            {{ activeFilterCount }} filter{{ activeFilterCount !== 1 ? 's' : '' }} active
          </span>
          <Button v-if="hasActiveFilters" label="Clear All" icon="pi pi-times" severity="danger" text size="small" @click="clearFilters" />
        </div>
      </div>
      ...
      />
      <p v-else class="text-sm text-gray-400 text-center py-8">No fields available for filtering.</p>

      <div class="flex justify-end gap-2 pt-3 border-t border-gray-200 mt-3">
        <Button label="Cancel" severity="secondary" outlined size="small" @click="showFilterPanel = false" />
        <Button label="Apply" severity="primary" size="small" @click="applyFilters" />
      </div>
    </div>
  ...
  >
    ...
    <template #footer>
      <Button label="Cancel" severity="secondary" outlined @click="closeDeleteModal" />
