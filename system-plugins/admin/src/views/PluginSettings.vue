...
onMounted(async () => {
  loading.value = true
  try {
    const { settings, schema: fetchedSchema } = await store.fetchPluginSettings(pluginName)
    if (fetchedSchema) {
    ...
    toast.show(`Failed to load settings: ${e instanceof Error ? e.message : 'Unknown error'}`, 'error')
  } finally {
    loading.value = false
  }
})
...
async function saveSettings() {
  saving.value = true
  try {
    await store.savePluginSettings(pluginName, formValues as Record<string, unknown>)
...
<template>
  <div class="p-6 max-w-2xl">
    <router-link :to="`/plugins/${pluginName}/`" class="inline-block mb-4 text-blue-500 text-sm hover:underline">← Back to Detail</router-link>

    <h1 class="text-2xl font-bold mb-6">Settings: {{ pluginName }}</h1>

    <div v-if="loading" class="p-8 text-center text-gray-500">Loading settings...</div>

    <!-- No schema warning -->
    <div v-else-if="!schema" class="p-6 bg-yellow-50 text-yellow-700 rounded-lg">
      This plugin does not have configurable settings.
    </div>

    <form v-else @submit.prevent="saveSettings" class="bg-white rounded-lg shadow-sm">
      ...
      <div class="p-4 bg-gray-50 rounded-b-lg flex gap-3">
        <button
          type="submit"
          :disabled="saving || Object.keys(formErrors).filter(k => formErrors[k]).length > 0"
