use axum::{
    Json, Router, extract::{Path, State}, http::{HeaderMap, StatusCode, header}, response::IntoResponse, routing::{delete, get, post}
};
...
use crate::{
    AppState,
    services::{
        self, context::{ ExtractContext},  postgres::{columntypetots::column_type_to_ts, inspector::{ DatabaseSchema, TableMeta}, tables::{FieldCreationObject, FieldMetaObject, FieldSavedMetaObject, FieldUpdateObject}}, respond::{self, JSendResponse, success}
    },
};
...
async fn get_schema(
    ...
    (StatusCode::OK, Json(success(DatabaseSchema { tables: schema.tables.clone(), columns: schema.columns.clone(), app_versions: schema.app_versions.clone() })))
}
...
async fn get_ts_schema(
    ...
    let version_name = match headers.get("x-version") {
        Some(version_value) => {
            ...
            match version_value.to_str() {
                ...
                Err(_) => 
                    return (
                            StatusCode::BAD_REQUEST, [(header::CONTENT_TYPE, "text/text")],
    ...
    let schemas_from_version: Vec<_> = schema.app_versions.iter()
    .filter(|av| av.version_name == version_name)
    .collect();

    for version in schemas_from_version {
        types += &format!("\t{}: {{\n",version.app_name);
        for table in schema.tables.iter().filter(|t| t.schema == version.schema_name) {
            let mut table_type = format!("\t\t{}:{{\n", table.name);

            for column in schema.columns.iter().filter(|col| col.schema == version.schema_name && col.table == table.name) {
                let ts_type = if let Some(fk) = &column.foreign_key {
                    let find_app = schema.tables.iter().find(|t| t.schema == fk.schema && t.name == fk.table).unwrap();
                    let find_version = schema.app_versions.iter().find(|av|av.schema_name == fk.schema).unwrap();

                    format!("{}|alcedo_apps[\"{}\"][\"{}\"]",column_type_to_ts(column.data_type.to_string()),find_version.app_name,find_app.name)
                }else {
...
async fn get_collections(
    State(state): State<AppState>,
) -> (StatusCode, Json<JSendResponse<Vec<GetCollectionsResponse>>>) {
    let schema = state.database_schema.read().await;
    ...
    .map(|table| {
        let find_version = schema.app_versions.iter().find(|v| {
            v.schema_name == table.schema
        }).unwrap();
        ...
        GetCollectionsResponse {
            ...
            fields: schema.columns.iter().filter(|field| {
                table.name == field.table && field.schema == table.schema
            }).map(|field| {
                let data_type = match field.data_type.as_str() {
                    ...
                    "timestamp" | "timestamp without time zone" | "timestamp with time zone" => CollectionFieldType::DateTime,
    ...
    (StatusCode::OK, Json(success(collections)))
}
...
async fn create_collection(
    ...
    match manager
        ...
        .await
    {
        Ok(_) => {Json(respond::success::<String>("Table created".to_string()))},
        Err(e) => {
            eprintln!("Error: {}", e);
            e.to_json_response()
        }
...
async fn drop_collection(
    ...
    match manager.drop_table(&name,&mut None).await {
        Ok(_) => Json(respond::success::<String>("Table dropped".to_string())),
        Err(e) => {
            eprintln!("Error: {}", e);
            e.to_json_response()
        }
...
async fn add_field(
    ...
    match manager.add_field(&collection, req.field,Some(req.meta),&mut None).await {
        Ok(_) => Json(respond::success::<String>("Field added".to_string())),
        Err(e) => e.to_json_response(),
    }
}
...
async fn drop_field(
    ...
    match manager.drop_field(&table, &field,&mut None).await {
        Ok(_) => Json(respond::success::<String>("Field dropped".to_string())),
        Err(e) => e.to_json_response(),
    }
}
...
async fn update_field(
    ...
    match manager.update_field(&table, &field, req.schema,req.meta,&mut None).await {
        Ok(_) => Json(respond::success::<String>("Field updated".to_string())),
        Err(e) => e.to_json_response(),
    }
}
