# Agent Development Guide - plugin-core

## Development vs Release Mode

plugin-core supports two build modes with different networking behaviors:
...
| **plugin-core Port** | Auto-detected (8081+ if 8080 is in use) |
...
| **plugin-core Port** | 8080 (configured) |
...
./target/release/plugin-core
```

### Docker Compose (full stack)

```bash
# Start everything (builds plugin-core image, starts postgres, redis, etc.)
...
| `CORE_PORT` | Port for plugin-core | 8080 |
...
The `test-k8s-setup/` directory contains manifests for deploying plugin-core on K3s/K3d.
...
kubectl port-forward -n plugin-core svc/plugin-core 6060:8080 --address 0.0.0.0
```
...
```json
{"read": true, "create": true, "update": true, "delete": true, "fields": ["field1", "field2"]}
```
