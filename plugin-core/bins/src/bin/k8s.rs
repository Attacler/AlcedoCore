...
async fn main() -> Result<(), AppError> {
    ...
    let platform: Arc<dyn PluginPlatform> = Arc::new(K8sPlatform::new().await?);

    // Deploy system plugins from remote manifest if configured
    if let Some(ref url) = config.system_plugins_url {
        tracing::info!("[SYSTEM_DEPLOYER] System plugin deployment skipped (K8s mode)");
    }
    ...
    // K8s event watcher is integrated into the platform
