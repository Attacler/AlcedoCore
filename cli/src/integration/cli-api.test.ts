...
// ---------------------------------------------------------------------------
// Dev session endpoints are only mounted when AlcedoCore runs in dev mode.
...
describe("Dev session API", () => {
    ...
    beforeAll(async () => {
        ...
        devEndpointsAvailable = await endpointAvailable(
            "/api/dev/start",
            "POST",
        );
    });
    ...
    it("unregisters a dev session via POST /api/dev/stop", async () => {
        ...
        const result = await apiCall<{ stopped: boolean }>("/api/dev/stop", {
            method: "POST",
            body: { slug: TEST_SLUG },
        });
