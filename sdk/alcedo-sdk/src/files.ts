export function createFilesResource(ky: any) {
  return {
    ...
    list: (params?: { search?: string; mime_type?: string; folder_id?: string; limit?: number; offset?: number }) =>
      ky.get('files', { searchParams: params as Record<string, string> }).json(),

    update: (id: string, data: { alt_text?: string; filename?: string; folder_id?: string }) =>
    ...
    folders: {
      ...
      delete: (id: string, recursive?: boolean) =>
        ky.delete(`files/folders/${id}`, { searchParams: recursive ? { recursive: 'true' } : undefined }).json(),
    },
  }
}
