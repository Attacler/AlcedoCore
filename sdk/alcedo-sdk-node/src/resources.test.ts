...
function getLastBody(ky: ReturnType<typeof mockKy>): any {
  ...
  if (ky.delete.mock.calls.length > 0) return ky.delete.mock.calls[0][1]?.json;
  return undefined;
}
...
describe("Error classes", () => {
  it("AlcedoError has correct name and properties", () => {
    const err = new AlcedoError(404, "Not Found", "test message", "ERR001");
    expect(err.name).toBe("AlcedoError");
  ...
  it("ConnectionError extends AlcedoError and carries original error", () => {
    const original = new Error("ECONNREFUSED");
    const err = new ConnectionError("connection refused", 0, original);
    expect(err.name).toBe("ConnectionError");
    expect(err.status).toBe(0);
    expect(err.original).toBe(original);
    expect(err).toBeInstanceOf(AlcedoError);
  });

  it("NotFoundError includes key", () => {
    const err = new NotFoundError("key not found", "my-key");
  ...
  it("ValidationError includes details", () => {
    ...
    const err = new ValidationError("invalid input", 422, details);
  ...
  it("ServerError has status 500 by default", () => {
...
describe("DB Resource", () => {
  it("query sends SQL with params and returns json", async () => {
    ...
    const result = await db.query("my-plugin", "SELECT * FROM t", [1], 60, 500);
...
describe("Dev Resource", () => {
  it("start sends slug, url, ttl", async () => {
    ...
    const result = await dev.start("my-plugin", "http://localhost:3000", 7200);
