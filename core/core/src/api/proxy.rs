...
pub async fn proxy_handler(
    ...
    tracing::info!("[PROXY] slug='{}' path='{}'", path_info.slug, path_info.path);
    ...
    if let Some(ref dev_registry) = state.dev_registry {
        if let Some(session) = dev_registry.get(&path_info.slug).await {
            tracing::info!("[PROXY] Dev session active for {} → {}", path_info.slug, session.url);
            let url_str = format!(
                "{}/{}",
            ...
            let body_bytes = axum::body::to_bytes(
                ...
                10_000_000,
            ).await.map_err(|e| AppError::Internal(format!("Failed to read body: {}", e)))?;
            ...
            match req_builder.send().await {
                Ok(response) => {
                    ...
                    return Ok(builder.body(axum::body::Body::from(body_bytes.to_vec()))
                        .map_err(|e| AppError::Internal(format!("Failed to build response: {}", e)))?);
                }
                Err(e) => {
                    tracing::error!("[PROXY] Dev session proxy failed: {} (target: {})", e, url_str);
                    return Err(AppError::Internal(format!("Failed to proxy request to dev server: {}", e)));
                }
            }
        }
    }
    ...
    let (container_id, _version, endpoint_count) = 'cache: {
        ...
        tracing::debug!("[PROXY] Cache miss for {}, falling back to DB", path_info.slug);
        ...
        let active_version = match PluginVersion::find_active(db_pool, &path_info.slug).await {
            ...
            Ok(None) => return Err(AppError::NotFound(
                format!("No active version found for plugin: {}", path_info.slug))),
            Err(e) => return Err(AppError::Internal(format!("Database error: {}", e))),
        };

        let cid = active_version.container_id.clone()
        ...
        let plugin = match crate::db::queries::Plugin::find_by_slug(db_pool, &path_info.slug).await {
            Ok(Some(p)) => p,
            Ok(None) => return Err(AppError::NotFound(
                format!("Plugin not found: {}", path_info.slug))),
            Err(e) => return Err(AppError::Internal(format!("Database error: {}", e))),
        };

        let ep_count = plugin.endpoints.as_array().map(|arr| arr.len()).unwrap_or(0);
        (cid, active_version.version, ep_count)
    };

    if endpoint_count == 0 {
        tracing::debug!("[PROXY] Plugin {} has no registered endpoints (proxying anyway)", path_info.slug);
    }
    ...
    let (target_url, parsed_url) = if state.dev_mode {
        let core_port = std::env::var("CORE_PORT").unwrap_or_else(|_| "8080".to_string());
        let url_str = format!("http://localhost:{}/{}", core_port, path_info.path);
        tracing::info!("[PROXY] dev mode, connecting to localhost");
        let url = reqwest::Url::parse(&url_str).map_err(|e| AppError::Internal(format!("Invalid URI: {}", e)))?;
        (url_str, url)
    } else {
    ...
    let (captured_body, captured_headers, captured_body_size) = if state.logging_channel.is_some() {
        ...
        if decision.should_capture {
            ...
            (Some(body_str), Some(headers_json), Some(body_bytes.len() as i32))
        } else {
            (None, None, None)
        }
    } else {
    ...
    match req_builder.send().await {
        Ok(response) => {
            ...
            Ok(builder.body(axum::body::Body::from(body_bytes.to_vec()))
                .map_err(|e| AppError::Internal(format!("Failed to build response: {}", e)))?)
        }
        Err(e) => {
            tracing::error!("[PROXY] Request failed: {} (target: {})", e, target_url);
            Err(AppError::Internal(format!("Failed to proxy request: {}", e)))
        }
    }
}
