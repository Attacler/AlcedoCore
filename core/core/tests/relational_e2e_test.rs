...
async fn create_coll(pool: &PgPool, name: &str, fields: Vec<FieldDefinition>) {
    ...
    sqlx::query("INSERT INTO collection_definitions (name, display_name) VALUES ($1, $2)")
        ...
        .await
        .unwrap();
    plugin_core::db::fields::replace_fields_in_tx(&mut tx, name, &fields)
        .await
        .unwrap();
...
async fn test_e2e_nested_field_selection() {
    ...
    create_coll(
        ...
        vec![
            ...
            FieldDefinition {
                ...
                is_system: false,
                hidden: false,
                related_collection: Some("e2e_authors".into()),
                relationship_type: Some("many_to_one".into()),
                display_field: None,
                inline_parent_fields: None,
            },
        ],
    ...
    let all = plugin_core::db::collections::list_collections(p)
        .await
        .unwrap();
    ...
    .unwrap();
    assert!(
        frags2.iter().any(|f| f.alias == "author_id"),
        "Wildcard should resolve author_id"
    );
...
async fn test_e2e_relational_crud() {
    ...
    create_coll(
        ...
        vec![
            ...
            FieldDefinition {
                ...
                is_system: false,
                hidden: false,
                related_collection: Some("e2e_orgs".into()),
                relationship_type: Some("many_to_one".into()),
                display_field: None,
                inline_parent_fields: None,
            },
    ...
    let mut items = vec![json!({"name": "Bob", "org_id": {"name": "Acme"}})
        .as_object()
        .unwrap()
        .clone()];
    ...
    tx.commit().await.unwrap();
    assert!(
        items[0]["org_id"].as_str().is_some(),
        "org_id should be UUID"
    );
    ...
    let results = plugin_core::db::collection_items::create_items(
        p,
        "e2e_contacts",
        CreateItemsBody::Single(items[0].clone()),
    )
    .await
    .unwrap();
...
async fn test_e2e_display_field() {
    ...
    create_coll(
        ...
        vec![
            ...
            FieldDefinition {
                ...
                is_system: false,
                hidden: false,
                related_collection: Some("e2e_depts".into()),
                relationship_type: Some("many_to_one".into()),
    ...
    let items = collection_items::query_items(
        p,
        "e2e_emps",
    ...
    let has_dv = items
        .iter()
        .any(|item| item.get("dept_id__display_value").is_some());
...
async fn test_e2e_relational_sections() {
    ...
    assert_eq!(
        rows[0].1, "children.parent",
        "relation_field should be namespaced '<collection>.<field>'"
    );
...
async fn test_e2e_parent_field_inlining() {
    ...
    create_coll(
        ...
        vec![
            ...
            FieldDefinition {
                ...
                is_system: false,
                hidden: false,
                related_collection: Some("e2e_companies".into()),
                relationship_type: Some("many_to_one".into()),
                display_field: None,
    ...
    assert!(
        f.inline_parent_fields.is_some(),
        "inline_parent_fields should be Some"
    );
