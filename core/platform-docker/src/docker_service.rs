...
impl DockerService for DockerServiceImpl {
    ...
    async fn list_tasks(&self) -> Result<Vec<JsonValue>, AppError> {
        ...
        let result: Vec<JsonValue> = tasks
            .into_iter()
            .map(|t| {
                ...
                if let Some(ref status) = t.status {
                    if let Some(ref container_status) = status.container_status {
                        map.insert(
                            ...
                            serde_json::json!(container_status.container_id),
                        );
                    }
                }
    ...
    async fn create_plugin_service(
        ...
        services::create_plugin_service(&DOCKER, slug, image, env, network_name, None, replicas)
            .await
    }
    ...
    async fn pull_image(&self, image: &str, registry: &Registry) -> Result<String, AppError> {
        DockerClient.pull_image(image, registry).await
    }
    ...
    async fn list_directory_in_image(
        ...
        DockerClient
            .list_directory_in_image(registry, image_name, dir_path)
            .await
    }
