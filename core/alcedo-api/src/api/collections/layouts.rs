...
pub(crate) async fn update_layout(
    ...
    if let Some(d) = body.is_default {
        if d {
            sqlx::query(
                "UPDATE collection_layouts SET is_default = false WHERE collection_name = $1",
            )
            .bind(&name)
            .execute(db_pool)
            .await
...
pub(crate) async fn delete_layout(
    ...
    let count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM collection_layouts WHERE collection_name = $1")
            .bind(&name)
            .fetch_one(db_pool)
            .await
            .map_err(|e| AppError::DatabaseError {
    ...
    let result =
        sqlx::query("DELETE FROM collection_layouts WHERE id::text = $1 AND collection_name = $2")
            .bind(&layout_id)
            .bind(&name)
            .execute(db_pool)
            .await
...
pub(crate) async fn get_layout_roles(
    ...
    let roles: Vec<Value> = rows
        .into_iter()
        .map(|r| json!({ "role_id": r.0, "role_name": r.1 }))
        .collect();
...
pub(crate) async fn set_layout_roles(
    ...
    if !body.role_ids.is_empty() {
        ...
        let role_count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM roles WHERE id::text = ANY($1)")
                .bind(&ids_refs)
                .fetch_one(db_pool)
                .await
                .map_err(|e| AppError::DatabaseError {
