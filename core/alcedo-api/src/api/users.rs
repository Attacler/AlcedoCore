...
async fn resolve_user_variables(
    ...
    if let Some(uid) = permission_check::extract_user_id_from_session(state, headers).await? {
        let context =
            permission_check::build_user_context(pool, &state.redis_connection, &uid, permissions)
                .await?;
...
pub async fn list_users_handler(
    ...
    let pc =
        permission_check::check_permission(&state, &identity, &headers, "users", "read").await?;
    match pc {
        ...
        PermissionCheck::Bypass => {
            ...
            let data: Vec<Value> = collection_items::query_items(pool, "users", query)
                .await?
                .into_iter()
                .map(|item| {
        ...
        PermissionCheck::Granted {
            ...
            let data: Vec<Value> = items
                ...
                .filter_map(|item| {
                    ...
                    let perms =
                        permission_check::compute_item_permissions_sync(&item, &resolved, false);
...
pub async fn create_user_handler(
    ...
    let pc =
        permission_check::check_permission(&state, &identity, &headers, "users", "create").await?;
    match pc {
        PermissionCheck::Denied { reason } => return Err(AppError::Forbidden(reason)),
        PermissionCheck::Bypass => {
            ...
            data.insert(
                "is_admin".to_string(),
                json!(payload.is_admin.unwrap_or(false)),
            );
            ...
            let user_email = result
                ...
                .to_string();
            let actor_id =
                crate::api::permission_check::extract_user_id_from_session(&state, &headers)
                    .await?
...
pub async fn update_user_handler(
    ...
    let pc =
        permission_check::check_permission(&state, &identity, &headers, "users", "update").await?;
    match pc {
        PermissionCheck::Denied { reason } => return Err(AppError::Forbidden(reason)),
        PermissionCheck::Bypass => {
            ...
            if payload.is_admin == Some(false) {
                let admin_count: i64 = sqlx::query_scalar(&format!(
                    "SELECT COUNT(*) FROM users WHERE is_admin = true and id != '{}'",
                    id
                ))
                .fetch_one(pool)
                .await
        ...
        PermissionCheck::Granted {
            ...
            if let Some(ref email) = payload.email {
                if allowed
                    .as_ref()
                    .map_or(true, |a| a.contains(&"email".to_string()))
                {
                    update.insert("email".to_string(), json!(email));
                }
            }
            if let Some(ref dn) = payload.display_name {
                if allowed
                    .as_ref()
                    .map_or(true, |a| a.contains(&"display_name".to_string()))
                {
                    update.insert("display_name".to_string(), json!(dn));
                }
            }
...
pub async fn delete_user_handler(
    ...
    let pc =
        permission_check::check_permission(&state, &identity, &headers, "users", "delete").await?;
    match pc {
        PermissionCheck::Denied { reason } => return Err(AppError::Forbidden(reason)),
        PermissionCheck::Bypass => {
            ...
            {
                let admin_count: i64 =
                    sqlx::query_scalar(r#"SELECT COUNT(*) FROM users WHERE is_admin = true"#)
                        .fetch_one(pool)
                        .await
                        .unwrap_or(0);
...
pub async fn change_password_handler(
    ...
    let is_admin = permission_check::require_scope(&state, &headers, "users.all")
        .await
        .is_ok();
