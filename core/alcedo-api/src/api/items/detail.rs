...
pub async fn get_item_handler(
    ...
    let pc =
        permission_check::check_permission(&state, &identity, &headers, &collection_name, "read")
            .await?;
    ...
    let fields_param: Vec<String> = params
        .get("fields")
        .map(|f| {
            f.split(',')
    ...
    let all_perms =
        permission_check::load_all_user_permissions(&state, &identity, &headers, &collection_name)
            .await?;
    ...
    {
        Ok(collection) => {
            let all_display_fields: Vec<&crate::db::collections::FieldDefinition> = collection
                .fields
                .iter()
                .filter(|f| {
                    f.field_type == crate::db::collections::FieldType::Relationship
                        && f.display_field.is_some()
                })
                .collect();
            let surviving_fields: Vec<_> = all_display_fields
                .into_iter()
                .filter(|f| {
                    restricted
                        .as_object()
                        .map_or(false, |obj| obj.contains_key(&f.name))
                })
                .collect();
            if surviving_fields.is_empty() {
                ...
                .await
                .unwrap_or_else(|_| vec![restricted.clone()]);
    ...
    .await
    {
        Ok(collection) => {
            ...
            if file_fields.is_empty() {
                ...
                .await
                .unwrap_or_else(|_| vec![final_item.clone()]);
    ...
    let perms = permission_check::compute_item_permissions(
        &final_item,
        &all_perms,
        is_admin,
        Some(db_pool),
...
pub async fn references_handler(
    ...
    let _pc =
        permission_check::require_permission(&state, &identity, &headers, &collection_name, "read")
            .await?;
        ...
        match ref_pc {
            ...
            PermissionCheck::Granted {
                ...
                let items: Vec<Value> = group
                    ...
                    .collect();
                filtered_references
                    .push(crate::db::collections::ReferencingGroup { items, ..group });
            }
        }
    }
