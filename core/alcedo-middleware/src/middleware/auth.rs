...
fn is_public_path(path: &str) -> bool {
    ...
    if path.starts_with("/api/plugins/") {
        ...
        if rest == "pages"
            || rest.ends_with("/pages")
            || rest == "pages/assets"
            || rest.ends_with("/pages/assets")
        {
            return true;
        }
    }
...
fn resource_permission(path: &str) -> Option<&'static str> {
    if path == "/api/roles" || path.starts_with("/api/roles/") {
        Some("roles")
    } else if path.starts_with("/api/plugins") {
        Some("plugins")
    } else if path.starts_with("/api/settings") {
        Some("settings")
    } else if path.starts_with("/api/menus") {
        Some("settings")
    } else if path.starts_with("/api/registries") {
        Some("plugins")
    } else if path.starts_with("/api/kv") {
        Some("kv")
    } else if path.starts_with("/api/policies") {
        Some("policies")
    } else if path.starts_with("/api/collections") {
        Some("collections")
    } else if path.starts_with("/api/logs") {
        Some("plugins")
    } else if path.starts_with("/api/dev") {
        Some("plugins")
...
pub async fn auth_middleware(
        ...
        if let Some(token) = auth_header.strip_prefix("Bearer ") {
            if let Some(ref db_pool) = state.db_pool {
                    ...
                    crate::db::queries::DeveloperApiKey::find_by_prefix(db_pool, prefix).await
                {
                    for key in keys {
                        if let Ok(true) =
                            crate::services::auth::verify_password(token, &key.key_hash).await
                        {
                            let _ = crate::db::queries::DeveloperApiKey::touch_last_used(
                                db_pool, key.id,
                            )
                            .await;
                            request
                                .extensions_mut()
                                .insert(crate::error::AuthLevel::DeveloperApiKey);
                            request
                                .headers_mut()
                                .insert("x-alcedo-root", axum::http::HeaderValue::from_static("1"));
    ...
    let session_id =
        extract_session_id(&request).and_then(|s| s.parse::<tower_sessions::session::Id>().ok());
        ...
        if let Some(ref pool) = state.db_pool {
            if let Ok(Some(user)) = crate::services::auth::find_user_by_id(pool, uid).await {
                if user.is_admin {
                    request
                        .extensions_mut()
                        .insert(crate::error::AuthLevel::Admin);
                } else {
                    request
                        .extensions_mut()
                        .insert(crate::error::AuthLevel::Authenticated);
                }
            } else {
    ...
    request
        .extensions_mut()
        .insert(alcedo_common::RequestIdentity {
            user_id,
