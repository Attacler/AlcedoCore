...
/// Drop all UNIQUE constraints currently defined on a given column.
///
/// Used when toggling `unique` off (or before re-adding with a canonical
/// constraint) so stale constraint names — whether auto-generated by
/// PostgreSQL (`{table}_{column}_key`) or explicitly named — are always
/// cleaned up regardless of how the constraint was originally created.
pub async fn drop_unique_constraints_on_column(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    table_name: &str,
    column_name: &str,
    ...
    .map_err(|e| AppError::DatabaseError {
        details: format!(
            "Failed to find unique constraints on {}.{}: {}",
    ...
    for name in &names {
        ...
        .await
        .map_err(|e| AppError::DatabaseError {
            details: format!("Failed to drop unique constraint '{}': {}", name, e),
        })?;
    }
    Ok(names.len())
}
