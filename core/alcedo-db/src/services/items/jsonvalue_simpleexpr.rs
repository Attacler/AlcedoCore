...
use alcedo_common::state::CoreDatabaseSchema;

use crate::services::items::query::TableJoin;
    ...
    let sea_value = match column.data_type.as_str() {
        ...
        "date" | "time" | "timetz" | "timestamp" | "timestamptz" | "interval" => {
            ...
            match s.as_str() {
                ...
                _ => sea_query::Value::String(value.as_str().map(|s| Box::new(s.to_string()))),
            }
        }
        ...
        "box" | "circle" | "line" | "lseg" | "path" | "point" | "polygon" => {
            sea_query::Value::String(value.as_str().map(|s| Box::new(s.to_string())))
        }
