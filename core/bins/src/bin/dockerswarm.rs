...
use pcl::events::{spawn_cache_invalidator, spawn_collection_log_writer, spawn_system_log_writer, spawn_event_forwarder, EventBus};
use pcl::plugins::health::AppState;
use pcl::providers::{RegistriesProvider, PluginContainerProvider};
...
async fn main() -> Result<(), AppError> {
    ...
    DockerServiceImpl.create_network_if_missing(&config.plugin_network).await?;
    tracing::info!("Ensured Docker network '{}' exists", config.plugin_network);

    let db_url = config.database_url.as_deref()
        .filter(|u| !u.is_empty())
        .ok_or_else(|| AppError::Internal("DATABASE_URL is required but was not set or is empty".to_string()))?;
    ...
    if let Some(ref pool) = db_pool {
        match CoreMigrationRunner::new(pool.clone()).run_pending().await {
            ...
            Ok(executed) => {
                tracing::info!("Applied {} core migration(s): {}", executed.len(), executed.join(", "));
            }
            Err(e) => {
                tracing::error!("Core migration failed: {}. Startup continuing without migrations.", e);
            }
        }
    }
    ...
    if let Some(ref pool) = db_pool {
        ...
        if user_count == 0 {
            if let (Some(ref admin_email), Some(ref admin_password)) = (&config.admin_email, &config.admin_password) {
                match pcl::services::auth::create_user(
                    ...
                    Ok(user) => {
                        tracing::info!("[AUTH] Created admin user: {} (id={})", user.email, user.id);
                    }
                    Err(e) => {
            ...
            tracing::info!("[AUTH] Users exist in database (count={}), skipping admin bootstrap", user_count);
        }
    }

    // Seed public role
    if let Some(ref pool) = db_pool {
        let public_role_exists: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM roles WHERE name = 'public')"
        )
        .fetch_one(pool)
        .await
        .unwrap_or(false);
    ...
    // Seed default roles
    if let Some(ref pool) = db_pool {
        let admin_role_exists: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM roles WHERE name = 'admin')"
        )
        .fetch_one(pool)
        .await
        .unwrap_or(false);
        ...
        if !admin_role_exists {
            ...
            tracing::info!("[RBAC] Seeded admin role with {} permissions", all_permissions.len());

            if let (Some(ref admin_email), _) = (&config.admin_email, &config.admin_password) {
                if let Ok(Some(user)) = pcl::services::auth::find_user_by_email(pool, admin_email).await {
    ...
    let docker_platform: Arc<dyn PluginPlatform> = Arc::new(
        DockerPlatform::new(
            db_pool.clone(),
            runtime.clone(),
    ...
    let file_sync_service = Arc::new(FileSyncServiceImpl::new(runtime.clone())) as Arc<dyn FileSyncService>;
    ...
    if let Some(ref url) = config.system_plugins_url {
        if let (Some(ref pool), Some(ref provider)) = (db_pool.as_ref(), plugin_containers_provider.as_ref()) {
    ...
    let rate_limit_redis: Option<Arc<tokio::sync::Mutex<redis::aio::ConnectionManager>>> = if redis_connection.is_some() {
    ...
    let kv_redis: Option<Arc<tokio::sync::Mutex<redis::aio::ConnectionManager>>> = if redis_connection.is_some() {
        match make_redis_conn().await {
            Ok(conn) => {
    ...
    docker_service.ensure_overlay_network(
    ...
    let kv_store: Arc<KvStore> = if let Some(ref redis_conn) = kv_redis {
        ...
        if let Ok(mut conn) = pool.get().await {
            ...
            tracing::warn!("Failed to get Redis connection from pool — KV store will return errors");
    ...
    let session_redis = redis::Client::open(config.redis_url.clone())
        ...
        .await
        .map_err(|e| AppError::Internal(format!("Failed to connect Redis for session store: {}", e)))?;
    ...
    let session_layer = tower_sessions::SessionManagerLayer::new(session_store.clone())
        ...
        .with_http_only(true)
        .with_secure(false)
        .with_expiry(tower_sessions::Expiry::OnInactivity(time::Duration::seconds(config.session_ttl_seconds as i64)));

    let file_storage_provider = std::env::var("FILE_STORAGE_PROVIDER")
    ...
    let file_storage: Arc<dyn FileStorage> = match file_storage_provider.as_str() {
        "s3" => {
            ...
            let storage = S3FileStorage::new(&bucket, &prefix).await
        ...
        "local" | _ => {
            ...
            let storage = LocalFileStorage::new(&base_path)
    ...
    if let Some(ref pool) = state.db_pool {
        ...
        spawn_event_forwarder(pool.clone(), rx, redis.clone(), config.event_forwarder_max_concurrent);
